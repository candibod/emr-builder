...
export function UserPopover({ anchorEl, onClose, open }: UserPopoverProps): React.JSX.Element {
  ...
  return (
    <Popover anchorEl={anchorEl} anchorOrigin={{ horizontal: "left", vertical: "bottom" }} onClose={onClose} open={open} slotProps={{ paper: { sx: { width: "240px" } } }}>
      <Box sx={{ p: "16px 20px " }}>
        <Typography variant="subtitle1">{typeof window !== "undefined" ? localStorage.getItem("user_full_name") : "User"}</Typography>
      ...
      <MenuList disablePadding sx={{ p: "8px", "& .MuiMenuItem-root": { borderRadius: 1 } }}>
        <MenuItem component={RouterLink} href={paths.builder.account} onClick={onClose}>
          <ListItemIcon>
            <PersonOutlineIcon />
          </ListItemIcon>
          Account
